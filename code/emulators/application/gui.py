...
if __name__ == "__main__":
    ...
    def simulate_pet_updates():
        ...
        for point in route:
            gui.update_pet_gps_coordinates(point)
            time.sleep(1)

    def simulate_user_updates():
        time.sleep(5)
        route = [
            (32.01487634797979, 34.77458326803195),
            (32.0149811, 34.7746679),
        ...
        for point in route:
            gui.set_user_gps_coordinates(point)
            time.sleep(1.2)

    threading.Thread(target=simulate_pet_updates).start()
    threading.Thread(target=simulate_user_updates).start()
